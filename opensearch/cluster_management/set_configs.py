...
def get_memory(client, instance_type):
    memory = client.describe_instance_types(InstanceTypes=[instance_type])[
        "InstanceTypes"
...
def get_jvm_file(new_memory, configs):
    target_path = os.path.join(
        configs["store_dir"], configs["identifier"] + ".jvm.options"
    )
...
def get_yml_file(manager_ip, node_count, configs, instance_type, roles):
    target_path = os.path.join(
        configs["store_dir"], configs["identifier"] + f".{instance_type}.opensearch.yml"
    )
    with open("opensearch.yml", "r") as yml_file:
        ...
        content = content.replace("node.roles: []", f"""node.roles: {roles}""")
...
async def set_configs(configs):
    ...
    manager_ip = get_instances_info(list(ids.values())[0], configs)["PrivateIpAddress"][
        0
    ]
    ...
    for instance_type, roles in configs["InstanceRolesMap"].items():
        yml_fp = get_yml_file(
            f"ip-{manager_ip}", node_count, configs, instance_type, roles
        )
