...
def get_ImageId(configs):
    ...
    response = client.describe_images(
        Filters=filters, Owners=["099720109477"]
    )  # Ubuntu's owner ID
...
def create_instances(configs):
    ...
    for instance_type, instance_count in configs["InstanceMap"].items():
        instance_args = {
            ...
            "TagSpecifications": [
                {"ResourceType": "instance", "Tags": configs["Tags"]},
            ],
        }

        if "IamInstanceProfile" in configs:
            instance_args["IamInstanceProfile"] = configs["IamInstanceProfile"]
        instances = ec2_res.create_instances(**instance_args)
